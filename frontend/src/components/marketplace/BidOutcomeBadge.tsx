...
// не заводим. «Ожидает»/«Подведены итоги» намеренно делят один нейтральный
// цвет — «Подведены итоги» сознательно не несёт исход (WCAG 1.4.1 тут не при
// чём: цвет не различает не потому, что дублирует текст, а потому что сам
// текст по решению продукта не должен намекать на исход).
const BID_OUTCOME_VARS: Record<BidOutcomeLabel, { bg: string; color: string }> = {
  ...
  "Подведены итоги":      { bg: "var(--ds-done-bg)",   color: "var(--ds-done-text)"   },
};
